...
class JavaDatasetReader(DatasetReader):
    ...
    def _read(self, file_path: str):
        ...
        modified_rules = self._optimize_grammar([d['rules'] for d in dataset])
    ...
    def _optimize_grammar(self, rules_lst):
        """ The two following optimizations are applied to the grammar.
         Firstly, the identifier rule is split based on parent state
         prefix to cut down the search space.  Secondly, the
         MethodDeclaration rule is simplified to reduce decoding
         steps. """

        IDENTIFIER_TYPES = ['Primary', 'ClassOrInterfaceType', 'Nt_33']
        new_rules = []
        for rules in rules_lst:
            # We need a stack here since multiple identifier
            # rules can be expanded and we need to keep track of the
            # parent nonterminal that generated it
            stack = []
            new_rules.append([])
            for rule in rules:
                lhs, rhs = rule.split('-->')
                if "MethodDeclaration" in lhs:
                    new_rule = rule.replace(IdentifierNT, 'function')
                    new_rule = new_rule.replace('MethodBody', '{___Star_26___}')
                    new_rules[-1].append(new_rule)
                elif "MethodBody" in lhs or rule == 'IdentifierNT-->function' or rule =='Block-->{___Star_26___}':
                    continue
                elif IdentifierNT in rhs:
                    stack.append(lhs)
